...
export const registerUser = async (req:Request,res:Response): Promise<Response> => {
    ...
    try {
        ...
        if (!_.isEmpty(user)) return res.status(400).json({message:'user already exist'});
...
export const loginUser = async (req:Request,res:Response) : Promise<Response> => {
    ...
    try {
        const user:QueryResult = await pool.query(`SELECT * FROM users WHERE email = $1`,[email])
        if(_.isEmpty(user)) return res.status(400).json({message: 'email not registered'})
        if(!decryptPwd(password,user.rows[0].password)) return res.status(400).json('invalid password')
        const token = generateToken({userId: user.rows[0].id},{expiresIn:'1h'})
